"""
~5M Capacity-Scaling Recovery Experiment — Benchmark Evaluation
==============================================================
Evaluates the 20 trained 5M-parameter recovery checkpoints on the frozen test sets.

Methodology is identical to 126K baseline, 1M experiment, and original 5M experiment:
...
- Outputs to results_5m_recovery/raw_benchmark_results.json
"""
...
if __name__ == '__main__':
    ...
    print(f"Executing 5M Recovery Benchmark Evaluation on device: {device}")

    # Verify checkpoints exist
    for task in TASKS:
        for seed in SEEDS:
            for mt in ['cot', 'latent']:
                cp = CHECKPOINT_BASE / f'{mt}_{task}_s{seed}_recovery' / 'weights.pth'
                assert cp.exists(), f"Checkpoint missing: {cp}"
    print(f"[OK] All 20 recovery checkpoints verified in {CHECKPOINT_BASE}")
    ...
    for task in TASKS:
        ...
        print(f"\n{'='*60}")
        print(f"BENCHMARKING 5M RECOVERY MODELS — TASK: {task.upper()} ({NUM_TEST_SAMPLES} fixed test instances)")
        print(f"{'='*60}")

        for seed in SEEDS:
            # laod kro 5M CoT Recovery (prefer best_weights.pth if present, fallback ko weights.pth)
            cot_dir = CHECKPOINT_BASE / f'cot_{task}_s{seed}_recovery'
            cot_path = cot_dir / 'best_weights.pth' if (cot_dir / 'best_weights.pth').exists() else cot_dir / 'weights.pth'
            cot = AutoregressiveCoT(**COT_5M_KWARGS).to(device)
            cot.load_state_dict(torch.load(cot_path, map_location=device))
            cot.eval()

            # laod kro 5M Latent Recovery
            latent_dir = CHECKPOINT_BASE / f'latent_{task}_s{seed}_recovery'
            latent_path = latent_dir / 'best_weights.pth' if (latent_dir / 'best_weights.pth').exists() else latent_dir / 'weights.pth'
            ...
            for k in BUDGETS:
                ...
                print(f"  CoT-5M-Rec  | k={k:2d} | Acc: {cot_acc*100:6.2f}% | Lat: {cot_lat_mean:6.2f}ms | FLOPs: {cot_flops:,.0f}")
                ...
                print(f"  Latent-5M-R | k={k:2d} | Acc: {lat_acc*100:6.2f}% | Lat: {lat_lat_mean:6.2f}ms | FLOPs: {lat_flops:,.0f}")
    ...
    print(f"\n{'='*60}")
    print(f"[OK] 5M Recovery Benchmark complete: 120 raw records written to {raw_path}")
    print(f"{'='*60}")

...
export const ParetoInstrument: React.FC = () => {
  ...
  const buildPoints = (): ParetoPoint[] => {
    ...
    // Compute empirical Pareto optimality (non-dominated points within the active set)
    // Point A dominates Point B if latency(A) <= latency(B) and acc(A) >= acc(B), with at least one strict inequality.
    points.forEach((pA) => {
  ...
  // Find points available within current latency budget cursor
  ...
  // Viewport dimensions for SVG Coordinate Plane
  const svgWidth = 720;
  const svgHeight = 360;
  const padLeft = 60;
  const padRight = 30;
  ...
  // Domain: Latency 0.0 to 1.3 ms, Accuracy 0% to 105%
  const maxLat = 1.30;
  const maxAcc = 105;
    ...
    <div className="workbench-viewport" style={{ border: '1px solid var(--border-medium)' }}>
      ...
      <div style={{ position: 'relative', background: 'var(--bg-viewport)' }}>
        ...
        >
          ...
          {[0, 20, 40, 60, 80, 100].map((acc) => (
            <g key={`grid-y-${acc}`}>
              <line
                ...
                y2={scaleY(acc)}
                stroke={acc === 0 || acc === 100 ? 'rgba(255, 255, 255, 0.15)' : 'rgba(255, 255, 255, 0.05)'}
                strokeWidth="1"
              />
              <text
                x={padLeft - 8}
          ...
          {/* Interactive Latency Hairline Cursor */}
          <line
          ...
          {points.map((p) => {
            ...
            return (
              ...
              >
                ...
                {/* Point Shape: Blue Square for CoT, Green Circle for Latent */}
                {isCoT ? (
                  <rect
                    ...
                    fill={isSelected ? '#f8fafc' : 'var(--cot-primary)'}
                    stroke="#080b11"
                    strokeWidth="1.5"
                  />
                ) : (
                  <circle
                    ...
                    cy={cy}
                    r={isSelected ? 6 : 4.5}
                    fill={isSelected ? '#f8fafc' : 'var(--latent-primary)'}
                    stroke="#080b11"
                    strokeWidth="1.5"
                  />
                )}
        ...
        {/* Draggable Latency Hairline Slider */}
        <div style={{
      ...
      {/* Latency Budget Interrogation Deck */}
      <div style={{
        ...
        {/* Left: What does the cursor find? */}
        <div style={{
          ...
          {selectedPoint ? (
            <div style={{ marginTop: '0.4rem', fontFamily: 'var(--font-mono)', fontSize: '0.82rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '2px', color: 'var(--text-tertiary)' }}>
                ...
                <span style={{ color: selectedPoint.isParetoOptimal ? '#f59e0b' : 'var(--text-tertiary)' }}>
                ...
                {points.map((p) => (
                  <tr
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      background: selectedPoint?.id === p.id ? 'rgba(56, 189, 248, 0.1)' : 'transparent',
                    }}
                  >
                    ...
                    <td style={{ padding: '6px', color: p.isParetoOptimal ? '#f59e0b' : 'var(--text-tertiary)' }}>

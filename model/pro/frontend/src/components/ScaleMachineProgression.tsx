...
export const ScaleMachineProgression: React.FC = () => {
  ...
  // Helper to extract accuracy for active model at selected budget k
  ...
  // X axis: budgets k in [1, 2, 4, 8, 12, 16]
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', padding: '1.25rem 2rem' }}>
      {/* 1. Header with Breadcrumbs, Title & Top-Right Model Scale Badge */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.75rem', color: '#64748b', marginBottom: '0.25rem', fontFamily: 'var(--font-mono)' }}>
            ...
            <span>&gt;</span>
            <span style={{ color: '#94a3b8' }}>Scale Machine</span>
          </div>
        ...
        >
          ...
          <div>
            <div style={{ fontSize: '0.78rem', fontWeight: 700, color: '#f1f5f9' }}>OBSERVED CONDITIONS</div>
            <div style={{ fontSize: '0.66rem', color: '#64748b' }}>Controlled small-scale study (RTX 4060)</div>
          </div>
        </div>
      </div>

      {/* 2. Hierarchical Model Scale Tree Selector (Prompt Section 3 & 4) */}
      <div
        style={{
          display: 'flex',
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
          <span style={{ fontSize: '0.72rem', color: '#64748b', fontFamily: 'var(--font-mono)', fontWeight: 700 }}>
            CAPACITY TREE:
          </span>

          {/* 126K Baseline Button */}
          <button
            onClick={() => setSelectedScale('126k')}
            style={{
              background: selectedScale === '126k' ? 'rgba(56, 189, 248, 0.15)' : '#070b12',
              border: selectedScale === '126k' ? '1px solid #38bdf8' : '1px solid #1a2538',
              color: selectedScale === '126k' ? '#38bdf8' : '#cbd5e1',
              borderRadius: '4px',
          ...
          >
            ...
            <span style={{ fontSize: '0.62rem', color: '#64748b' }}>Scaling condition</span>
          </button>

          <span style={{ color: '#475569', fontSize: '0.8rem', fontWeight: 700 }}>→</span>

          {/* 1M Scaling Button */}
          <button
            onClick={() => setSelectedScale('1m')}
            style={{
              background: selectedScale === '1m' ? 'rgba(56, 189, 248, 0.15)' : '#070b12',
              border: selectedScale === '1m' ? '1px solid #38bdf8' : '1px solid #1a2538',
              color: selectedScale === '1m' ? '#38bdf8' : '#cbd5e1',
              borderRadius: '4px',
              padding: '0.4rem 0.75rem',
              fontSize: '0.74rem',
          ...
          {/* 5M Architecture with Sub-branches */}
          <div
            style={{
              display: 'flex',
              flexDirection: 'column',
          ...
          >
            <div style={{ fontSize: '0.65rem', color: '#94a3b8', fontFamily: 'var(--font-mono)', fontWeight: 700 }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button
                ...
                style={{
                  background: selectedScale === '5m_original' ? 'rgba(239, 68, 68, 0.18)' : '#0a101d',
                  border: selectedScale === '5m_original' ? '1px solid #f87171' : '1px solid #291515',
                  color: selectedScale === '5m_original' ? '#f87171' : '#cbd5e1',
                  borderRadius: '3px',
              ...
              <button
                ...
                style={{
                  background: selectedScale === '5m_recovery' ? 'rgba(56, 189, 248, 0.18)' : '#0a101d',
                  border: selectedScale === '5m_recovery' ? '1px solid #38bdf8' : '1px solid #162a3f',
                  color: selectedScale === '5m_recovery' ? '#38bdf8' : '#cbd5e1',
                  borderRadius: '3px',
                  padding: '0.3rem 0.6rem',
                  fontSize: '0.7rem',
              ...
              >
                └── 5M Recovery <span style={{ fontSize: '0.62rem', color: '#38bdf8' }}>[Controlled Recovery]</span>
              </button>
            </div>
          </div>
        </div>

        {/* Budget k selector for the active scale */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
          <span style={{ fontSize: '0.72rem', color: '#64748b', fontFamily: 'var(--font-mono)' }}>BUDGET k:</span>
          <div style={{ display: 'flex', background: '#070b12', padding: '2px', borderRadius: '4px', border: '1px solid #1e293b' }}>
            {kValues.map((k) => (
              <button
                key={k}
                onClick={() => setSelectedBudgetK(k)}
                style={{
                  background: selectedBudgetK === k ? '#0284c7' : 'transparent',
                  color: selectedBudgetK === k ? '#ffffff' : '#94a3b8',
                  border: 'none',
                  borderRadius: '3px',
                  fontSize: '0.7rem',
      ...
      {/* 3. Main Grid: Empirical Accuracy Curve (Left) and Scale Inspector (Right) */}
      <div
        style={{
          display: 'grid',
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
              ...
              {/* Legend */}
              <div style={{ display: 'flex', gap: '0.85rem', fontSize: '0.7rem', color: '#94a3b8' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
                  <span style={{ width: '12px', borderTop: '1px dashed #64748b' }} />
            ...
            <div style={{ width: '100%', overflowX: 'auto' }}>
              <svg width="100%" height={svgHeight} viewBox={`0 0 ${svgWidth} ${svgHeight}`} style={{ overflow: 'visible' }}>
                ...
                {[0, 20, 40, 60, 80, 100].map((val) => {
                  ...
                  return (
                    <g key={val}>
                      <line x1={pad.left} y1={y} x2={svgWidth - pad.right} y2={y} stroke="#172234" strokeWidth="1" />
                      <text x={pad.left - 8} y={y + 3} textAnchor="end" fill="#64748b" fontSize="9" fontFamily="var(--font-mono)">
                        {val}%
                      </text>
                    </g>
                  );
                ...
                {kValues.map((k) => {
                  ...
                  return (
                    <g key={k}>
                      <line x1={x} y1={pad.top} x2={x} y2={pad.top + pHeight} stroke="#172234" strokeWidth="1" />
                      <text x={x} y={pad.top + pHeight + 16} textAnchor="middle" fill="#64748b" fontSize="9" fontFamily="var(--font-mono)">
                        k={k}
                      </text>
                    </g>
                  );
                  ...
                  return (
                    <g key={r.k} style={{ cursor: 'pointer' }} onClick={() => setSelectedBudgetK(r.k)}>
                      ...
                      <circle
                        ...
                        r={isSelectedK ? 6 : 4}
                        fill="#38bdf8"
                        stroke={isSelectedK ? '#ffffff' : '#090e17'}
                      ...
                      {/* Values at k=1 and k=16 */}
                      {(r.k === 1 || r.k === 16 || isSelectedK) && (
                        <>
                          <text x={x} y={yCoT - 8} textAnchor="middle" fill="#38bdf8" fontSize="9" fontWeight="700" fontFamily="var(--font-mono)">
                            {r.cot_acc.toFixed(1)}%
                          </text>
                          <text x={x} y={yLatent + 14} textAnchor="middle" fill="#10b981" fontSize="9" fontWeight="700" fontFamily="var(--font-mono)">
          ...
          {/* Core Empirical Findings Card (Section 4 & 6 of RESULTS_5M_RECOVERY.md) */}
          <div
            style={{
              backgroundColor: '#090e17',
              border: '1px solid #172234',
          ...
          >
            <div style={{ fontSize: '0.82rem', fontWeight: 800, color: '#f8fafc', marginBottom: '0.85rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              {[
                ...
                <div key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.65rem' }}>
                  ...
                  <div>
                    <div style={{ fontSize: '0.76rem', fontWeight: 700, color: '#f1f5f9' }}>{obs.title}</div>
                    <div style={{ fontSize: '0.72rem', color: '#94a3b8', lineHeight: 1.35 }}>{obs.desc}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Right Column: Model Inspector & Seed 46 Variance */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          >
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid #172234', gap: '0.5rem', marginBottom: '0.2rem' }}>
              <button
                ...
                style={{
                  ...
                  color: activeTab === 'inspector' ? '#38bdf8' : '#64748b',
                  fontSize: '0.76rem',
                  fontWeight: 700,
              ...
              <button
                ...
                style={{
                  background: 'transparent',
                  border: 'none',
                  borderBottom: activeTab === 'seeds' ? '2px solid #38bdf8' : '2px solid transparent',
                  color: activeTab === 'seeds' ? '#38bdf8' : '#64748b',
                  fontSize: '0.76rem',
                  fontWeight: 700,
                  padding: '0.35rem 0.5rem',
                  cursor: 'pointer',
            ...
            <div>
              ...
              <div style={{ fontSize: '0.68rem', color: activeSpec.badgeType === 'historical' ? '#f87171' : '#38bdf8', fontFamily: 'var(--font-mono)' }}>
                {activeSpec.badge}
              </div>
            </div>
            ...
            {activeTab === 'inspector' ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.45rem', fontSize: '0.74rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#64748b' }}>CoT Parameters</span>
                  <span style={{ color: '#e2e8f0', fontFamily: 'var(--font-mono)' }}>{activeSpec.paramsCoT.toLocaleString()}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#64748b' }}>Latent Parameters</span>
                  <span style={{ color: '#e2e8f0', fontFamily: 'var(--font-mono)' }}>{activeSpec.paramsLatent.toLocaleString()}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#64748b' }}>Layers</span>
                  <span style={{ color: '#e2e8f0', fontFamily: 'var(--font-mono)' }}>{activeSpec.layers}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#64748b' }}>Hidden Dim $d_{'{model}'}$</span>
                  <span style={{ color: '#e2e8f0', fontFamily: 'var(--font-mono)' }}>{activeSpec.hiddenDim}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#64748b' }}>Attention Heads</span>
                  <span style={{ color: '#e2e8f0', fontFamily: 'var(--font-mono)' }}>{activeSpec.heads}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#64748b' }}>Task A (k={selectedBudgetK}) CoT</span>
                  <span style={{ color: '#38bdf8', fontFamily: 'var(--font-mono)', fontWeight: 700 }}>{currentRecord.cot_acc.toFixed(2)}% (±{currentRecord.cot_sem.toFixed(2)})</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#64748b' }}>Task A (k={selectedBudgetK}) Latent</span>
                  <span style={{ color: '#10b981', fontFamily: 'var(--font-mono)', fontWeight: 700 }}>{currentRecord.latent_acc.toFixed(2)}% (±{currentRecord.latent_sem.toFixed(2)})</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#64748b' }}>Task B (k=16) CoT</span>
                  <span style={{ color: '#e2e8f0', fontFamily: 'var(--font-mono)' }}>{activeSpec.taskBCoTk16.toFixed(2)}%</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#64748b' }}>Task B (k=16) Latent</span>
                  <span style={{ color: '#e2e8f0', fontFamily: 'var(--font-mono)' }}>{activeSpec.taskBLatentk16.toFixed(2)}%</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#64748b' }}>Latency (k={selectedBudgetK}) CoT</span>
                  <span style={{ color: '#cbd5e1', fontFamily: 'var(--font-mono)' }}>{currentRecord.cot_latency_ms.toFixed(3)} ms</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: '#64748b' }}>Latency (k={selectedBudgetK}) Latent</span>
                  <span style={{ color: '#cbd5e1', fontFamily: 'var(--font-mono)' }}>{currentRecord.latent_latency_ms.toFixed(3)} ms</span>
                </div>
              </div>
            ) : (
              /* Seed Variance Tab */
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
                ...
                {selectedScale === '126k' && (
                  <div style={{ background: '#060a12', border: '1px solid #1e293b', borderRadius: '4px', padding: '0.6rem', fontSize: '0.7rem' }}>
                    <div style={{ color: '#38bdf8', fontWeight: 700, marginBottom: '0.35rem', fontFamily: 'var(--font-mono)' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '4px', textAlign: 'center', fontFamily: 'var(--font-mono)' }}>
                      <div style={{ background: '#0a101d', padding: '4px', borderRadius: '3px', border: '1px solid #172234' }}>
                        <div style={{ color: '#64748b', fontSize: '0.62rem' }}>SEED 42</div>
                        <div style={{ color: '#e2e8f0', fontWeight: 600 }}>76.20%</div>
                      </div>
                      <div style={{ background: '#0a101d', padding: '4px', borderRadius: '3px', border: '1px solid #172234' }}>
                        <div style={{ color: '#64748b', fontSize: '0.62rem' }}>SEED 43</div>
                        <div style={{ color: '#e2e8f0', fontWeight: 600 }}>69.70%</div>
                      </div>
                      <div style={{ background: '#0a101d', padding: '4px', borderRadius: '3px', border: '1px solid #172234' }}>
                        <div style={{ color: '#64748b', fontSize: '0.62rem' }}>SEED 44</div>
                        <div style={{ color: '#e2e8f0', fontWeight: 600 }}>58.90%</div>
                      </div>
                      <div style={{ background: '#0a101d', padding: '4px', borderRadius: '3px', border: '1px solid #172234' }}>
                        <div style={{ color: '#64748b', fontSize: '0.62rem' }}>SEED 45</div>
                        <div style={{ color: '#e2e8f0', fontWeight: 600 }}>62.90%</div>
                      </div>
                      <div style={{ background: 'rgba(245, 158, 11, 0.15)', padding: '4px', borderRadius: '3px', border: '1px solid rgba(245, 158, 11, 0.4)' }}>
                        <div style={{ color: '#f59e0b', fontSize: '0.62rem', fontWeight: 700 }}>SEED 46 ★</div>
                ...
                {activeSpec.seed46OutlierNote && selectedScale !== '126k' && (
                  <div style={{ background: 'rgba(245, 158, 11, 0.1)', border: '1px solid rgba(245, 158, 11, 0.3)', borderRadius: '4px', padding: '0.6rem', fontSize: '0.7rem', color: '#fde68a' }}>
                    <strong>Seed 46 Variance Note:</strong> {activeSpec.seed46OutlierNote}
                  </div>
                )}

                {selectedScale === '5m_recovery' && (
                  <div style={{ background: 'rgba(56, 189, 248, 0.08)', border: '1px solid #1e293b', borderRadius: '4px', padding: '0.6rem', fontSize: '0.7rem', color: '#cbd5e1' }}>
            ...
            <div style={{ borderTop: '1px solid #172234', paddingTop: '0.65rem' }}>
              <div style={{ fontSize: '0.68rem', color: '#64748b', fontFamily: 'var(--font-mono)', marginBottom: '0.25rem' }}>

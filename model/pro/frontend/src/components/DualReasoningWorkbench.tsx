...
export const DualReasoningWorkbench: React.FC<DualReasoningWorkbenchProps> = ({
  ...
  // Problem Setup: v0 = 3, D = 4, pi = [1, 7, 5, 0, 2, 4, 6, 3]
  // Traversal: 3 -> 0 -> 1 -> 7 -> 3
  const groundTruthTarget = 'v3';

  // CoT Emitted Token Objects for budgetK
  ...
  // Active CoT tokens determined by budgetK
  ...
  // Latent Continuous Recurrent State Objects for budgetK
  ...
  // Profiled telemetry per budget k
  ...
  return (
    <div className="workbench-viewport" style={{ border: '1px solid var(--border-medium)', background: 'var(--bg-surface)' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            ...
            {/* Selectable Continuous Latent Recurrent States */}
            <div style={{
              background: 'var(--bg-viewport)',
              border: '1px solid var(--border-subtle)',
              borderRadius: '3px',
      ...
      {/* Floating State Inspector if an Item is Selected */}
      {selectedItem && (
        <div style={{

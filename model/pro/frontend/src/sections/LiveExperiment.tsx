...
export const LiveExperiment: React.FC = () => {
  ...
  const runExperiment = async (k: number, exampleToRun: TaskExample) => {
    ...
    setErrorMsg(null);
    try {
      // Call FastAPI compare endpoint - evaluates BOTH models on the EXACT same instance
      ...
      setIsLiveApi(false);
      // Construct fallback simulation for initial offline preview
      const hops = exampleToRun.depth;
      const trueAns = exampleToRun.expected_answer;

      // CoT at k=2: emits 2 reasoning tokens; on depth 6, k=2 is insufficient -> incorrect
      const cotTokens = [

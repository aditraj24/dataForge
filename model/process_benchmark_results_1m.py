"""
~1M Capacity-Scaling Experiment — Result Processing & Analysis
==============================================================
Processes raw 1M benchmark results into aggregated statistics,
Pareto frontiers, paired statistical tests, and threshold analysis.

Reads from results_1m/raw_benchmark_results.json.
Writes to results_1m/{aggregated,pareto,statistical,threshold,RESULTS_1M.md}.
"""
...
from plot_pareto_frontiers_1m import generate_pareto_plots
...
print(f"Loaded {len(raw)} raw 1M evaluation records.")
...
report_lines = []
report_lines.append("# Results — 1M Capacity-Scaling Experiment")
report_lines.append("")
report_lines.append("## Architecture")
report_lines.append("| Model | d_model | num_layers | dim_feedforward | Parameters |")
report_lines.append("|-------|---------|------------|-----------------|------------|")
report_lines.append("| AutoregressiveCoT (1M) | 224 | 2 | 624 | 998,520 |")
report_lines.append("| RecurrentLatentReasoner (1M) | 224 | — | 1,027 | 998,523 |")
report_lines.append(f"| Parameter difference | | | | 3 (0.0003%) |")
report_lines.append("")
...
for task in ['task_a', 'task_b']:
    ...
    for entry in aggregated:
        ...
        k = entry['k']
        model = entry['model']
        if model == 'AutoregressiveCoT':
            # Find matching latent entry
            lat_entry = [e for e in aggregated if e['task'] == task and e['k'] == k and e['model'] == 'RecurrentLatentReasoner'][0]
            stat_entry = [s for s in paired_stats if s['task'] == task and s['k'] == k][0]
            report_lines.append(
                f"| {k} | {entry['mean_accuracy']:.4f} | ±{entry['sem_accuracy']:.4f} | "
                f"{lat_entry['mean_accuracy']:.4f} | ±{lat_entry['sem_accuracy']:.4f} | "
                f"{entry['mean_latency_ms']:.3f} | {lat_entry['mean_latency_ms']:.3f} | "
...
report_md = "\n".join(report_lines)
with open(RESULTS_DIR / 'RESULTS_1M.md', 'w', encoding='utf-8') as f:
    f.write(report_md)

print("Generating Pareto frontier plots...")
generate_pareto_plots(RESULTS_DIR)

print("Statistical processing complete! Artifacts written to results_1m/")
print(f"Files: aggregated_results.json, pareto_frontiers.json, statistical_comparison.json, threshold_analysis.json, RESULTS_1M.md, pareto_frontier_plots.png")

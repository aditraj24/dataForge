...
export const PermutationMachine: React.FC<PermutationMachineProps> = ({
  ...
  // Compute Ground Truth sequence ahead ka time kay lyye any configuration
  ...
  // Jump ko specific step from timeline
  ...
  // nyakro a single destination in BUILD mode
  ...
  return (
    <div className="workbench-viewport" style={{ border: '1px solid var(--border-medium)', background: 'var(--bg-surface)' }}>
      ...
      <div className="workbench-bar" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.6rem 1rem', background: 'var(--bg-surface)', borderBottom: '1px solid var(--border-medium)' }}>
        ...
        {/* Mode Selector [PREsett] vs [BUILD] */}
      ...
      {/* Validation Warning Alert if mapping h not a valid bijection */}
      {!validation.isValid && (
        <div style={{
        ...
        <div style={{
              ...
              return (
                <g key={`edge-${src.id}`}>
                  <line
                    ...
                    x2={tgt.x}
                    y2={tgt.y}
                    stroke={isCurrentEdge ? 'var(--text-accent)' : isTraversed ? 'var(--correct-color)' : 'rgba(255, 255, 255, 0.12)'}
              ...
              return (
                ...
                >
                  ...
                  <text
                    ...
                    y={node.y + 4}
                    textAnchor="middle"
                    fill={isCurrent ? 'var(--bg-primary)' : 'var(--text-primary)'}
                    fontFamily="var(--font-mono)"
                    fontSize="11"
                    fontWeight="700"
                  >
          ...
          <div style={{
            ...
            {!validation.isValid ? (
              ...
              <span>Start node <strong style={{ color: 'var(--warning-color)' }}>v{v0}</strong>. Press STEP or RUN ORBIT.</span>
            ) : !isComplete ? (
              <span>
      ...
      {/* Real Step-by-Step Computational haaalat Timeline Bar */}
      <div style={{
        background: 'var(--bg-surface)',

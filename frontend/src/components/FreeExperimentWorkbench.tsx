...
export const FreeExperimentWorkbench: React.FC = () => {
  ...
  // Task A parmeters (Permutation Orbit)
  ...
  // Task B parmeters (Modular Register)
  ...
  // Inspection mode kay lyye Latent maadal: [ MACHINE VIEW ] vs [ 3D haaalat SPACE ]
  ...
  // Ground-truth computation kay lyye Task A
  ...
  // Ground-truth computation kay lyye Task B
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', padding: '1.25rem 2rem' }}>
      {/* 1. Header sath Breadcrumb, Badges & Title */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.75rem', color: 'var(--text-tertiary)', marginBottom: '0.25rem', fontFamily: 'var(--font-mono)' }}>
            ...
            <span>&gt;</span>
            <span style={{ color: 'var(--text-secondary)' }}>Free Experiment</span>
            <span>&gt;</span>
          ...
          <p style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', margin: '0.35rem 0 0 0', maxWidth: '820px', lineHeight: 1.45 }}>
            Construct custom problem instances for <strong>Task A (Permutation Orbit)</strong> or <strong>Task B (Modular Register)</strong>. Compare how <strong>Autoregressive Scratchpad</strong> and <strong>Recurrent Latent Core</strong> execute the inference steps under variable thinking budgets <code style={{ color: 'var(--text-accent)' }}>k</code>.
          </p>
        </div>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <FlaskConical size={13} style={{ color: 'var(--text-accent)' }} />
            <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-primary)' }}>SANDBOX RUNNER</span>
          </div>
          <div style={{ fontSize: '0.7rem', color: 'var(--text-secondary)' }}>
            Target Model: <strong style={{ color: modelType === 'cot' ? 'var(--text-accent)' : '#a855f7' }}>{modelType === 'cot' ? 'CoT Scratchpad' : 'Recurrent Latent'} ({modelScale.toUpperCase()})</strong>
          </div>
          <div style={{ fontSize: '0.68rem', color: 'var(--text-tertiary)' }}>
            Status: <span style={{ color: isRunning ? 'var(--warning-color)' : 'var(--correct-color)' }}>{isRunning ? 'Executing forward pass...' : executionTimestamp}</span>
          </div>
        </div>
      </div>
      ...
      {modelType === 'latent' && taskType === 'task_a' && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--bg-surface)', padding: '0.5rem 1rem', borderRadius: '6px', border: '1px solid var(--border-subtle)' }}>
          ...
          <div style={{ display: 'flex', background: 'var(--bg-surface-elevated)', padding: '2px', borderRadius: '4px', border: '1px solid var(--border-medium)' }}>
            <button
              onClick={() => setLatentViewMode('machine')}
              style={{
                background: latentViewMode === 'machine' ? 'var(--text-accent)' : 'transparent',
                color: latentViewMode === 'machine' ? 'var(--text-primary)' : 'var(--text-secondary)',
                border: 'none',
                borderRadius: '3px',
                fontSize: '0.7rem',
            ...
            <button
              ...
              style={{
                background: latentViewMode === '3d' ? 'var(--text-accent)' : 'transparent',
                color: latentViewMode === '3d' ? 'var(--text-primary)' : 'var(--text-secondary)',
                border: 'none',
                borderRadius: '3px',
                fontSize: '0.7rem',
                padding: '0.3rem 0.75rem',
      ...
      {/* If 3D inspection mode h open kay lyye Latent on Task A, display ThreehaaalatTrajectory */}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: '1rem', alignItems: 'stretch' }}>
          ...
          >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '4px' }}>
                <button
                  ...
                  style={{
                    ...
                    fontSize: '0.72rem',
                    fontWeight: 700,
                    background: taskType === 'task_a' ? 'var(--text-accent)' : 'var(--bg-primary)',
                    color: taskType === 'task_a' ? 'var(--text-primary)' : 'var(--text-secondary)',
                    border: '1px solid var(--border-subtle)',
                    borderRadius: '4px',
                    cursor: 'pointer',
                  }}
                ...
                <button
                  ...
                  style={{
                    ...
                    fontSize: '0.72rem',
                    fontWeight: 700,
                    background: taskType === 'task_b' ? 'var(--text-accent)' : 'var(--bg-primary)',
                    color: taskType === 'task_b' ? 'var(--text-primary)' : 'var(--text-secondary)',
                    border: '1px solid var(--border-subtle)',
                    borderRadius: '4px',
                    cursor: 'pointer',
                  }}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '4px' }}>
                <button
                  ...
                  style={{
                    ...
                    fontSize: '0.72rem',
                    fontWeight: 700,
                    background: modelType === 'cot' ? 'var(--text-accent)' : 'var(--bg-primary)',
                    color: modelType === 'cot' ? 'var(--text-primary)' : 'var(--text-secondary)',
                    border: '1px solid var(--border-subtle)',
                    borderRadius: '4px',
                    cursor: 'pointer',
                  }}
                ...
                <button
                  ...
                  style={{
                    ...
                    fontSize: '0.72rem',
                    fontWeight: 700,
                    background: modelType === 'latent' ? '#a855f7' : 'var(--bg-primary)',
                    color: modelType === 'latent' ? 'var(--text-primary)' : 'var(--text-secondary)',
                    border: '1px solid var(--border-subtle)',
                    borderRadius: '4px',
                    cursor: 'pointer',
                  }}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', borderTop: '1px solid var(--border-subtle)', paddingTop: '0.75rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', gap: '3px' }}>
                    <button onClick={() => loadPresetPermutation('standard')} style={{ background: 'var(--bg-primary)', border: '1px solid var(--border-medium)', color: 'var(--text-secondary)', fontSize: '0.62rem', padding: '1px 4px', borderRadius: '2px', cursor: 'pointer' }}>Standard</button>
                    <button onClick={() => loadPresetPermutation('cycle')} style={{ background: 'var(--bg-primary)', border: '1px solid var(--border-medium)', color: 'var(--text-secondary)', fontSize: '0.62rem', padding: '1px 4px', borderRadius: '2px', cursor: 'pointer' }}>Cycle</button>
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(8, 1fr)', gap: '4px' }}>
                  {permutation.map((target, src) => (
                    <div key={src} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px' }}>
                      <span style={{ fontSize: '0.62rem', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)' }}>v{src}</span>
                      <input
                        type="number"
                        min="0"
                        max="7"
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '0.6rem' }}>
                  <div>
                    <span style={{ fontSize: '0.68rem', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)' }}>START NODE:</span>
                    <select
                      value={startNode}
                      onChange={(e) => setStartNode(parseInt(e.target.value))}
                      style={{ width: '100%', background: 'var(--bg-surface-elevated)', border: '1px solid var(--border-medium)', color: 'var(--text-primary)', fontSize: '0.74rem', padding: '0.35rem', borderRadius: '4px' }}
                    >
                  ...
                  <div>
                    <span style={{ fontSize: '0.68rem', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)' }}>DEPTH D ({depthD} hops):</span>
                    <input
                      type="range"
                      min="1"
                      max="8"
                ...
                {/* Compute bjjet k */}
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.68rem', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)' }}>
                    <span>THINKING BUDGET (k):</span>
                    <span style={{ color: 'var(--warning-color)', fontWeight: 700 }}>k = {budgetK}</span>
                  </div>
                  <input
                    type="range"
                    min="1"
            ...
            {taskType === 'task_b' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', borderTop: '1px solid var(--border-subtle)', paddingTop: '0.75rem' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '0.6rem' }}>
                  <div>
                    <span style={{ fontSize: '0.68rem', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)' }}>INITIAL REG (R0):</span>
                    <input
                      type="number"
                      value={initialReg}
                  ...
                  <div>
                    <span style={{ fontSize: '0.68rem', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)' }}>MODULUS (M):</span>
                    <input
                      type="number"
                      min="2"
                      max="100"
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  {operations.map((item, idx) => (
                    <div key={idx} style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                      <span style={{ fontSize: '0.68rem', color: 'var(--text-tertiary)', width: '22px' }}>#{idx + 1}</span>
                      <select
                        value={item.op}
                        onChange={(e) => {
                        ...
                        style={{ background: 'var(--bg-surface-elevated)', border: '1px solid var(--border-medium)', color: 'var(--text-primary)', fontSize: '0.7rem', padding: '2px 4px', borderRadius: '3px' }}
                      >
                      ...
                      <input
                        ...
                        style={{ width: '50px', background: 'var(--bg-surface-elevated)', border: '1px solid var(--border-medium)', color: 'var(--warning-color)', fontSize: '0.72rem', textAlign: 'center', padding: '2px', borderRadius: '3px' }}
                      />
                      <span style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)' }}>(mod {modulus})</span>
                    </div>
                  ))}
                </div>
            ...
            <div style={{ marginTop: 'auto', display: 'flex', gap: '0.5rem' }}>
              ...
              >
                <Play size={13} fill="var(--text-primary)" />
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-subtle)', paddingBottom: '0.65rem' }}>
              <div>
                <div style={{ fontSize: '0.85rem', fontWeight: 800, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <Binary size={15} style={{ color: 'var(--text-accent)' }} />
            ...
            {!hasRun ? (
              ...
              >
                <Play size={24} style={{ color: 'var(--text-accent)', opacity: 0.6 }} />
                <div style={{ fontWeight: 700, color: 'var(--text-secondary)', letterSpacing: '0.05em' }}>NO RUN YET</div>
                <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', maxWidth: '340px', lineHeight: 1.45 }}>
            ...
            {taskType === 'task_a' && modelType === 'cot' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', background: 'var(--bg-surface-elevated)', border: '1px solid var(--border-subtle)', borderRadius: '4px', padding: '1rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.72rem', fontFamily: 'var(--font-mono)' }}>
                    <span style={{ color: 'var(--text-tertiary)', width: '90px' }}>INPUT:</span>
                    <span style={{ color: 'var(--text-secondary)', background: '#0a1220', padding: '2px 8px', borderRadius: '3px', border: '1px solid var(--border-medium)' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.72rem', fontFamily: 'var(--font-mono)' }}>
                    <span style={{ color: 'var(--text-tertiary)', width: '90px' }}>EMBEDDING:</span>
                    <span style={{ color: 'var(--text-secondary)' }}>Tokenized vocabulary indices [0..7, HOP, QUERY]</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'center', color: 'var(--text-muted)' }}>↓</div>

                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.72rem', fontFamily: 'var(--font-mono)' }}>
                    <span style={{ color: 'var(--text-tertiary)', width: '90px' }}>TRANSFORMER:</span>
                    <span style={{ color: 'var(--text-accent)' }}>Causal masked multi-head self-attention ({modelScale.toUpperCase()})</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'center', color: 'var(--text-muted)' }}>↓</div>

                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.72rem', fontFamily: 'var(--font-mono)' }}>
                    <span style={{ color: 'var(--text-tertiary)', width: '90px' }}>GENERATION:</span>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                      <span style={{ color: '#a855f7', background: 'rgba(168, 85, 247, 0.15)', padding: '2px 6px', borderRadius: '3px' }}>&lt;THINK&gt;</span>
                      {taskAOrbit.map((v, i) => (
                        <React.Fragment key={i}>
                          <span style={{ color: 'var(--text-accent)', background: 'var(--bg-surface-elevated)', padding: '2px 6px', borderRadius: '3px' }}>v{v}</span>
                          {i < taskAOrbit.length - 1 && <span style={{ color: 'var(--text-tertiary)' }}>→</span>}
                        </React.Fragment>
                      ))}
                      <span style={{ color: 'var(--correct-color)', background: 'var(--latent-bg)', padding: '2px 6px', borderRadius: '3px' }}>&lt;ANSWER&gt; v{targetAnswerA}</span>
                    </div>
                  </div>
                </div>

                {/* sahi expectation callout based on empirical benchmarks */}
                <div style={{ background: 'var(--bg-surface-elevated)', border: '1px solid var(--border-medium)', borderRadius: '4px', padding: '0.75rem', fontSize: '0.72rem', color: 'var(--text-secondary)' }}>
                  <div style={{ color: 'var(--text-accent)', fontWeight: 700, marginBottom: '0.2rem' }}>EMPIRICAL BEHAVIOR ON TASK A:</div>
                  CoT accuracy scales steadily with budget <code style={{ color: 'var(--warning-color)' }}>k</code>. At 5M Recovery, CoT achieves <strong>99.74%</strong> at k=16 (seed mean). With depth D={depthD} and budget k={budgetK}, scratchpad retains complete teacher-forced intermediate token history.
                </div>
              </div>
            )}
            ...
            {taskType === 'task_a' && modelType === 'latent' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', background: 'var(--bg-surface-elevated)', border: '1px solid var(--border-subtle)', borderRadius: '4px', padding: '1rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.72rem', fontFamily: 'var(--font-mono)' }}>
                    <span style={{ color: 'var(--text-tertiary)', width: '80px' }}>INPUT:</span>
                    <span style={{ color: 'var(--text-secondary)' }}>Table π, start=v{startNode}, D={depthD}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'center', color: 'var(--text-muted)' }}>↓</div>

                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.72rem', fontFamily: 'var(--font-mono)' }}>
                    <span style={{ color: 'var(--text-tertiary)', width: '80px' }}>MEMORY:</span>
                    <span style={{ color: 'var(--text-accent)' }}>Static cross-attention key/value memory table</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'center', color: 'var(--text-muted)' }}>↓</div>

                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.72rem', fontFamily: 'var(--font-mono)' }}>
                    <span style={{ color: 'var(--text-tertiary)', width: '80px' }}>QUERY:</span>
                    <span style={{ color: 'var(--warning-color)' }}>Initial recurrent state h_0 = Embed(v{startNode})</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'center', color: 'var(--text-muted)' }}>↓</div>

                  {/* Recurrent Steps */}
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.72rem', fontFamily: 'var(--font-mono)' }}>
                    <span style={{ color: 'var(--text-tertiary)', width: '80px' }}>RECURSION:</span>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                      ...
                      {budgetK > 8 && <span style={{ color: 'var(--text-tertiary)' }}>... S{budgetK}</span>}
                    </div>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'center', color: 'var(--text-muted)' }}>↓</div>

                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.72rem', fontFamily: 'var(--font-mono)' }}>
                    <span style={{ color: 'var(--text-tertiary)', width: '80px' }}>ANSWER:</span>
                    <span style={{ color: 'var(--correct-color)', fontWeight: 700 }}>Linear Readout Head(h_{budgetK}) → v{targetAnswerA}</span>
                  </div>
                </div>
            ...
            {taskType === 'task_b' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                <div style={{ fontSize: '0.72rem', color: 'var(--text-secondary)' }}>
                  Modular Register Machine transitions: Initial register <code style={{ color: 'var(--text-accent)' }}>R₀ = {initialReg}</code> evaluated under modulus <code style={{ color: 'var(--warning-color)' }}>M = {modulus}</code>.
                </div>

                {/* Transition steps table */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', background: 'var(--bg-surface-elevated)', border: '1px solid var(--border-subtle)', borderRadius: '4px', padding: '0.75rem' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '40px 140px 80px 80px', fontSize: '0.67rem', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)', paddingBottom: '0.3rem', borderBottom: '1px solid var(--border-subtle)' }}>
                    <span>STEP</span>
                    <span>OPERATION</span>
                    <span>PREV REG</span>
                    <span>NEXT REG</span>
                  ...
                  {taskBTransitions.history.map((h) => (
                    <div key={h.step} style={{ display: 'grid', gridTemplateColumns: '40px 140px 80px 80px', fontSize: '0.72rem', fontFamily: 'var(--font-mono)', padding: '0.3rem 0', borderBottom: '1px solid #0d1522' }}>
                      <span style={{ color: 'var(--text-tertiary)' }}>#{h.step}</span>
                      <span style={{ color: 'var(--text-primary)', fontWeight: 600 }}>{h.op}</span>
                      <span style={{ color: 'var(--text-secondary)' }}>{h.prev}</span>
                      <span style={{ color: 'var(--text-accent)', fontWeight: 700 }}>{h.result}</span>
                    </div>
                  ))}
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.5rem', paddingTop: '0.5rem', borderTop: '1px solid var(--border-subtle)', fontSize: '0.74rem', fontFamily: 'var(--font-mono)' }}>
                    <span style={{ color: 'var(--text-tertiary)' }}>FINAL COMPUTED REGISTER:</span>
                    <span style={{ color: 'var(--correct-color)', fontWeight: 800, fontSize: '0.88rem' }}>R_final = {taskBTransitions.finalReg}</span>
                  </div>
                </div>

...
export const ThreeStateTrajectory: React.FC = () => {
  ...
  const [activeCamera, setActiveCamera] = useState<'orbit' | 'top' | 'side' | 'front'>('orbit');
  ...
  const handleCameraPreset = (preset: 'orbit' | 'top' | 'side' | 'front' | 'reset') => {
    setActiveCamera(preset === 'reset' ? 'orbit' : preset);
  ...
  useEffect(() => {
    ...
    // Subtle 3D Bounding Box Wireframe sath axes
    ...
    TRAJECTORY_DATA.forEach((pt, i) => {
      ...
      let color = 0xa855f7; // purple "other haaalats"
  ...
  }, [selectedIndex, showTrajectory, showStartEnd, viewMode2D, activeCamera]);

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', padding: '1.25rem 2rem' }}>
      {/* 1. View Header sath Breadcrumb, Title & Top-Right maadal Badge */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.75rem', color: 'var(--text-tertiary)', marginBottom: '0.25rem', fontFamily: 'var(--font-mono)' }}>
            ...
            <span>&gt;</span>
            <span style={{ color: 'var(--text-secondary)' }}>Latent Workbench</span>
          </div>
        ...
        {/* Top-Right Badge Card (Task & maadal Selector) */}
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
          ...
          >
            ...
            <div>
              <div style={{ fontSize: '0.78rem', fontWeight: 700, color: 'var(--text-primary)' }}>Task A: Permutation Orbit</div>
              <div style={{ fontSize: '0.66rem', color: 'var(--text-accent)', fontFamily: 'var(--font-mono)' }}>PRECOMPUTED DIAGNOSTIC RUN</div>
            </div>
          </div>
          ...
          >
            <div style={{ fontSize: '0.66rem', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)' }}>Model Architecture</div>
            <select
              value={modelName}
            ...
            >
              <option value="Transformer (1M)" style={{ background: 'var(--bg-viewport)', color: 'var(--text-primary)' }}>1M Scaled [Precomputed Diagnostic]</option>
              <option value="Transformer (126K)" style={{ background: 'var(--bg-viewport)', color: 'var(--text-primary)' }}>126K Baseline [Precomputed Diagnostic]</option>
              <option value="Transformer (5M Recovery)" style={{ background: 'var(--bg-viewport)', color: 'var(--text-primary)' }}>5M Recovery [Precomputed Diagnostic]</option>
            </select>
          </div>
        </div>
      </div>
      ...
      >
        ...
        >
          <div style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-primary)', letterSpacing: '0.04em' }}>
            Visualization Controls
          </div>

          {/* Representation Pills */}
          <div>
            <div style={{ fontSize: '0.7rem', color: 'var(--text-tertiary)', marginBottom: '0.4rem', fontFamily: 'var(--font-mono)' }}>
              Representation
            </div>
            <div style={{ display: 'flex', gap: '3px', background: 'var(--bg-surface-elevated)', padding: '2px', borderRadius: '4px', border: '1px solid var(--border-subtle)' }}>
              {(['PCA', 't-SNE', 'UMAP'] as const).map((r) => (
                <button
                  ...
                  style={{
                    ...
                    fontSize: '0.72rem',
                    fontWeight: 600,
                    background: representation === r ? 'var(--text-accent)' : 'transparent',
                    color: representation === r ? 'var(--text-primary)' : 'var(--text-secondary)',
                    border: 'none',
                    borderRadius: '3px',
                    cursor: 'pointer',
                  }}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
            {[
              ...
              <label key={tg.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer' }}>
                <span style={{ fontSize: '0.74rem', color: 'var(--text-secondary)' }}>{tg.label}</span>
                <input
                  type="checkbox"
                  checked={tg.val}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '4px' }}>
              {(['Orbit', 'Top', 'Side', 'Front'] as const).map((cam) => (
                <button
                  ...
                  style={{
                    background: activeCamera === cam.toLowerCase() ? 'var(--text-accent)' : 'var(--bg-surface)',
                    border: '1px solid var(--border-medium)',
                    color: activeCamera === cam.toLowerCase() ? 'var(--text-primary)' : 'var(--text-secondary)',
                    fontSize: '0.7rem',
                    padding: '0.35rem 0',
                    borderRadius: '4px',
                    cursor: 'pointer',
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem', fontSize: '0.68rem', color: 'var(--text-secondary)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                <span style={{ width: '12px', height: '2px', background: 'var(--text-accent)', display: 'inline-block' }}></span>
                <span>Trajectory</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                <span style={{ width: '7px', height: '7px', borderRadius: '50%', background: 'var(--text-accent)', display: 'inline-block' }}></span>
        ...
        >
          ...
          <div style={{ display: 'flex', borderBottom: '1px solid var(--border-subtle)', gap: '0.5rem', marginBottom: '0.2rem' }}>
            <button
              ...
              style={{
                background: 'transparent',
                border: 'none',
                borderBottom: activeInspectorTab === 'inspector' ? '2px solid var(--text-accent)' : '2px solid transparent',
                color: activeInspectorTab === 'inspector' ? 'var(--text-accent)' : 'var(--text-tertiary)',
                fontSize: '0.76rem',
                fontWeight: 700,
            ...
            <button
              ...
              style={{
                background: 'transparent',
                border: 'none',
                borderBottom: activeInspectorTab === 'metrics' ? '2px solid var(--text-accent)' : '2px solid transparent',
                color: activeInspectorTab === 'metrics' ? 'var(--text-accent)' : 'var(--text-tertiary)',
                fontSize: '0.76rem',
                fontWeight: 700,
                padding: '0.35rem 0.5rem',
                cursor: 'pointer',
          ...
          {/* Active haaalat Header sath < > Stepper */}
            ...
            <div style={{ display: 'flex', gap: '4px' }}>
              ...
              <button
                ...
                style={{
                  background: 'var(--bg-surface-elevated)',
                  border: '1px solid var(--border-medium)',
                  color: selectedIndex === TRAJECTORY_DATA.length - 1 ? 'var(--text-muted)' : 'var(--text-primary)',
                  borderRadius: '3px',
                  width: '24px',
                  height: '24px',
                  display: 'flex',
          ...
          {activeInspectorTab === 'inspector' ? (
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.45rem', fontSize: '0.74rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: 'var(--text-tertiary)' }}>Step index</span>
                  <span style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-mono)' }}>{selectedState.step}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: 'var(--text-tertiary)' }}>Token</span>
                  <span style={{ color: 'var(--text-accent)', fontFamily: 'var(--font-mono)', fontWeight: 600 }}>{selectedState.token}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: 'var(--text-tertiary)' }}>Position (PC1, PC2, PC3)</span>
                  <span style={{ color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)', fontSize: '0.68rem' }}>
                    ({selectedState.x.toFixed(2)}, {selectedState.y.toFixed(2)}, {selectedState.z.toFixed(2)})
                  </span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: 'var(--text-tertiary)' }}>State norm ||h||₂</span>
                  <span style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-mono)' }}>{selectedState.norm.toFixed(2)}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: 'var(--text-tertiary)' }}>Δ(h_t, h_&#123;t-1&#125;)</span>
                  <span style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-mono)' }}>{selectedState.delta.toFixed(4)}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: 'var(--text-tertiary)' }}>cos(h_t, h_&#123;t-1&#125;)</span>
                  <span style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-mono)' }}>{selectedState.cosineSim.toFixed(3)}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: 'var(--text-tertiary)' }}>Top prediction</span>
                  <span style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-mono)' }}>{selectedState.prediction}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ color: 'var(--text-tertiary)' }}>Ground truth</span>
                  <span style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-mono)' }}>{selectedState.groundTruth}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              {/* Nearest haaalats sath Similarity Bars */}
              <div style={{ marginTop: '0.5rem', borderTop: '1px solid var(--border-subtle)', paddingTop: '0.65rem' }}>
                <div style={{ fontSize: '0.7rem', color: 'var(--text-tertiary)', marginBottom: '0.45rem', fontFamily: 'var(--font-mono)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                  {selectedState.nearestStates.map((ns) => (
                    <div key={ns.label} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.7rem' }}>
                      <span style={{ width: '22px', color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)' }}>{ns.label}</span>
                      <div style={{ flex: 1, height: '6px', background: 'var(--bg-viewport)', borderRadius: '3px', overflow: 'hidden' }}>
                        <div
                          style={{
                            width: `${ns.sim * 100}%`,
                            height: '100%',
                      ...
                      <span style={{ width: '30px', textAlign: 'right', color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)', fontSize: '0.67rem' }}>
                        {ns.sim.toFixed(2)}
                      </span>
                    </div>
                  ))}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', fontSize: '0.74rem' }}>
              <div style={{ background: 'var(--bg-surface-elevated)', border: '1px solid var(--border-medium)', borderRadius: '4px', padding: '0.6rem' }}>
                <div style={{ fontSize: '0.7rem', fontWeight: 700, color: 'var(--text-accent)', marginBottom: '0.3rem', fontFamily: 'var(--font-mono)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '55px 75px 1fr', fontSize: '0.66rem', color: 'var(--text-tertiary)', fontFamily: 'var(--font-mono)', paddingBottom: '0.2rem', borderBottom: '1px solid var(--border-subtle)' }}>
                  <span>STEP</span>
                  <span>Δ MAG</span>
                ...
                {TRANSITION_DELTAS.map((t) => (
                  <div key={t.transition} style={{ display: 'grid', gridTemplateColumns: '55px 75px 1fr', fontSize: '0.68rem', fontFamily: 'var(--font-mono)', padding: '0.2rem 0', borderBottom: '1px solid #0d1522' }}>
                    <span style={{ color: 'var(--text-secondary)', fontWeight: 600 }}>{t.transition}</span>
                    <span style={{ color: 'var(--warning-color)', fontWeight: 700 }}>{t.delta}</span>
                    <span style={{ color: 'var(--text-secondary)', fontSize: '0.64rem' }}>{t.desc}</span>
                  </div>
                ))}
              </div>

              <div style={{ fontSize: '0.65rem', color: 'var(--text-tertiary)', fontStyle: 'italic', lineHeight: 1.35, borderTop: '1px solid var(--border-subtle)', paddingTop: '0.45rem' }}>
      ...
      >
            ...
            <div style={{ display: 'flex', alignItems: 'center', flex: 1, position: 'relative', padding: '0 0.5rem' }}>
              ...
              <div
                style={{
                  ...
                  right: '10px',
                  height: '3px',
                  background: 'linear-gradient(90deg, var(--text-accent) 0%, var(--text-accent) 50%, #f97316 100%)',
                  zIndex: 1,
                }}
              />
                    ...
                    >
                      <div
                        style={{
                          ...
                          borderRadius: '50%',
                          backgroundColor: isSelected ? 'var(--warning-color)' : idx === 0 ? 'var(--text-accent)' : idx === TRAJECTORY_DATA.length - 1 ? '#f97316' : 'var(--text-accent)',
                          boxShadow: 'none',
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '0.45rem' }}>
            <button
              onClick={() => setIsPlaying(!isPlaying)}
              style={{
                display: 'flex',
            ...
            >
              <Play size={12} style={{ color: 'var(--text-accent)' }} />
              <span>Animate</span>
            </button>
            <button
              onClick={() => {
            ...
            >
              <Download size={12} style={{ color: 'var(--text-accent)' }} />
              <span>Export</span>
            </button>
            <button
            ...
            >
              <Columns size={12} style={{ color: 'var(--text-accent)' }} />
